...
if [ $# -lt 2 ]; then
    echo "requires arg 'VERTEX_ENDPOINT, VERTEX_MODEL_ID'"
    exit 1
fi
...
echo "endpoints['${VERTEX_ENDPOINT}'].undeploy(model = '${VERTEX_MODEL_ID}')"

HASH=$(sum <<< "${VERTEX_ENDPOINT}-${VERTEX_MODEL_ID}" | cut -f 1 -d ' ')
...
envsubst < ./undeploy_endpoint_model_job.yaml > "/tmp/undeploy_endpoint_model_job.${VERTEX_ENDPOINT}_${VERTEX_MODEL_ID}.yaml"

kubectl --context bolcom-pro-default --namespace reco-analytics apply --namespace reco-analytics -f - < "/tmp/undeploy_endpoint_model_job.${VERTEX_ENDPOINT}_${VERTEX_MODEL_ID}.yaml"
...
[[ "$LOGS" =~ .*"Endpoint model undeployed.".* ]] && {
  echo "endpoints['${VERTEX_ENDPOINT}'].undeploy(model = '${VERTEX_MODEL_ID}').ok"
  exit 0
}

echo "endpoints['${VERTEX_ENDPOINT}'].undeploy(model = '${VERTEX_MODEL_ID}').err"
exit 1
