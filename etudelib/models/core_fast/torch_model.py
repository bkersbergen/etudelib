...
class CORE_FASTModel(nn.Module):
    ...
    This modifies the algorithm with a few simple tweaks to speed it up
    """
    ...
    def __init__(self,
        ...
        super(CORE_FASTModel, self).__init__()
        ...
        # Construct normalized embeddings
        normalized_embeddings = F.normalize(self.item_embedding.weight.detach().transpose(0, 1).contiguous(), dim=-1)
        self.normalized_embeddings = nn.Parameter(normalized_embeddings, requires_grad=False)
    ...
    def forward(self, item_seq, _item_seq_len):
        ...
        if self.training:
            ...
            scores = torch.matmul(seq_output, self.normalized_embeddings)
            return scores
